...
resource "aws_s3_bucket" "s3_bucket_for_emr_logs" {
  bucket = "${var.prefix_analyzer}-analyzerng-emr-logs"
  acl    = var.acl
  lifecycle_rule {
    enabled = true
    expiration {
  ...
  tags = {
    BU              = var.bu
    Name            = "${var.prefix_analyzer}-analyzerng-emr-logs"
...
resource "aws_s3_bucket" "s3_bucket_for_emr2_logs" {
  bucket = "${var.prefix_analyzer}-analyzerng-emr-logparser-logs"
  acl    = var.acl
  lifecycle_rule {
    enabled = true
    expiration {
  ...
  tags = {
    BU              = var.bu
    Name            = "${var.prefix_analyzer}-analyzerng-emr-logparser-logs"
    Region          = var.tag_region
...
resource "aws_iam_role" "iam_emr_ec2_profile_role" {
  name = "${var.environment}-${var.name_analyzer}-Jobflow_role"
...
resource "aws_iam_role" "emr_service_role" {
  name               = "emr-${var.environment}-${var.name_analyzer}-ServiceRole"
...
#  vpc_id                 = data.terraform_remote_state.vpc.outputs.vpc_id
  ...
  tags = {
    Name            = "sg-${var.name_analyzer}-Master"
    BU              = var.bu
    Application     = var.application
    Environment     = var.environment
    Applicationtype = var.Applicationtype
...
#  vpc_id                 = data.terraform_remote_state.vpc.outputs.vpc_id
  ...
  tags = {
    Name            = "sg-${var.name_analyzer}-Slave"
    BU              = var.bu
    Application     = var.application
    Environment     = var.environment
    Applicationtype = var.Applicationtype
...
#    subnet_id                         = data.terraform_remote_state.vpc.outputs.vpc_public_subnets_ids[0]
  ...
  master_instance_fleet {
    instance_type_configs {
      ...
      ebs_config {
        size                 = var.ebs_size_analyzer
        type                 = var.ebs_type
        volumes_per_instance = var.volumes_per_instance
      }
      instance_type = var.master_instance_type_analyzer
    }
