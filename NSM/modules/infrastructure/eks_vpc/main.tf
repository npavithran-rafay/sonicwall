...
#subnet for gitea
resource "aws_subnet" "eks_gitea_new_private_subnets" {
  count = "${length(var.eks_gitea_private_az_subnets_cidr)}"

  cidr_block        = "${lookup(var.eks_gitea_private_az_subnets_cidr[count.index], "cidr")}"
  vpc_id            = "${aws_vpc.eks_vpc.id}"
  availability_zone = "${lookup(var.eks_gitea_private_az_subnets_cidr[count.index], "az")}"
  map_public_ip_on_launch = true

  tags = "${
    tomap({
      "Name" = "${lookup(var.eks_gitea_private_az_subnets_cidr[count.index], "name")}",
      "kubernetes.io/cluster/${var.eks_cluster_name}" = "shared",
      "kubernetes.io/role/internal-elb" = "1",
    })
  }"
...
#subnet for gitea
resource "aws_subnet" "eks_neo4j_new_private_subnets" {
  count = "${length(var.eks_neo4j_private_az_subnets_cidr)}"

  cidr_block        = "${lookup(var.eks_neo4j_private_az_subnets_cidr[count.index], "cidr")}"
  vpc_id            = "${aws_vpc.eks_vpc.id}"
  availability_zone = "${lookup(var.eks_neo4j_private_az_subnets_cidr[count.index], "az")}"
  map_public_ip_on_launch = true

  tags = "${
    tomap({
      "Name" = "${lookup(var.eks_neo4j_private_az_subnets_cidr[count.index], "name")}",
      "kubernetes.io/cluster/${var.eks_cluster_name}" = "shared",
      "kubernetes.io/role/internal-elb" = "1",
    })
  }"
...
resource "aws_route_table_association" "eks_neo4j_private_routetable_association" {
  count          = "${length(var.eks_neo4j_private_az_subnets_cidr)}"
  subnet_id      = "${element(aws_subnet.eks_neo4j_new_private_subnets.*.id, count.index)}"
  route_table_id = "${aws_route_table.production_private_routetable.id}"
}
resource "aws_route_table_association" "eks_gitea_private_routetable_association" {
  count          = "${length(var.eks_gitea_private_az_subnets_cidr)}"
  subnet_id      = "${element(aws_subnet.eks_gitea_new_private_subnets.*.id, count.index)}"
  route_table_id = "${aws_route_table.production_private_routetable.id}"
}